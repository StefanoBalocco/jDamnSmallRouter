...
// Il modulo accede a `window` solo al momento della prima chiamata a GetDamnSmallRouter(),
// non durante la valutazione del modulo: il mock va quindi impostato prima di quella chiamata.
const mockLocation: { hash: string } = { hash: '' };
...
// ── RouteAdd ──────────────────────────────────────────────────────────────────
// Paralleli: operazioni sincrone su path univoci.

{
	...
	test( prefix + ': throws SyntaxError for duplicate path id', ( t ) => {
		t.throws(
			() => router.RouteAdd( '/routeadd/:id/sub/:id', () => {} ),
	...
	test( prefix + ': throws SyntaxError for duplicate path id with bracket type', ( t ) => {
		t.throws(
			() => router.RouteAdd( '/routeadd/:id[09]/sub/:id[09]', () => {} ),
			{ instanceOf: SyntaxError }
		);
	} );
}
...

{
	...
	test( prefix + ': throws SyntaxError for duplicate path id', ( t ) => {
		t.throws(
			() => router.RouteDel( '/routedel/:id/sub/:id' ),
			{ instanceOf: SyntaxError }
		);
	} );
}
...
// ── Route ─────────────────────────────────────────────────────────────────────
// Tutti seriali: chiamano Route() che condivide il flag _routing con il singleton.
// Race condition inevitabile se eseguiti in parallelo.

{
	...
	test.serial( prefix + ': passes correct params', async ( t ) => {
		let received: { [ key: string ]: string } = {};
	...
	// Seriali: testano _routing e l'ordine della coda — incompatibili con routing concorrente.
...
// ── Trigger ───────────────────────────────────────────────────────────────────
// Seriali: modificano mockLocation.hash (stato condiviso globale).
...
// ── CheckHash ─────────────────────────────────────────────────────────────────
// Seriali: leggono/scrivono mockLocation.hash (stato condiviso globale).
